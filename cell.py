...
class Cell():
    ...
    def getNeighbors(self, settings):
        ...
        neighbors = {}
        neighbors['top'] = gf.getIndex(self.row - 1, self.col, settings.numRows, settings.numCols)
        neighbors['bottom'] = gf.getIndex(self.row + 1, self.col, settings.numRows, settings.numCols)
        neighbors['left'] = gf.getIndex(self.row, self.col - 1, settings.numRows, settings.numCols)
        neighbors['right'] = gf.getIndex(self.row, self.col + 1, settings.numRows, settings.numCols)
        return neighbors
